...
class ErrorCodesTest {
    ...
    class ErrorCodeConstruction{
        ...
        void doesNotThrowErrorOnInvalidInput(){
            assertNotNull(ErrorCodeUtils.createErrorResponseFromCode(-1));
        }
        @Test
        void createsCorrectErrorCode(){
            SubsonicResponse errorResponse = ErrorCodeUtils.createErrorResponseFromCode(70);
            assertEquals(70,errorResponse.childNodes.getFirst("error").attributes.get("code"));
        }
        @Test
        void ErrorRootObjectHasCorrectStatus(){
            SubsonicResponse response = ErrorCodeUtils.createErrorResponseFromCode(0);
